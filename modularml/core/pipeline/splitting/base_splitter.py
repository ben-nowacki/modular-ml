...
class BaseSplitter(ABC):
    ...
    def get_config(self) -> dict[str, Any]:
        """
        Return a serializable configuration for the splitter.

        Description:
            Should contain all parameters required to reproduce this
            splitter's behavior. For example, the seed, ratios, or
            stratification keys used in the split.
    ...
    @classmethod
    @abstractmethod
    def from_config(cls, config: Mapping[str, Any]) -> BaseSplitter:
        """
        Recreate a splitter instance from a configuration dictionary.

        Args:
            config (Mapping[str, Any]):
                The configuration produced by `get_config()`.

        Returns:
            BaseSplitter:
                A reconstructed splitter instance with identical behavior.
