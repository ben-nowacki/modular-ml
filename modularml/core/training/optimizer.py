...
class Optimizer(Configurable, Stateful):
    ...
    # def get_state(self) -> dict[str, Any]:
    #     """
    #     Full backend-agnostic serialization of the optimizer.

    #     Includes:
    #         - SerializableMixin base state
    #         - construction metadata
    #         - backend-specific internal optimizer state (if built)
    #     """
    #     state = {}

    #     # Identify optimizer class target (only needed if no name)
    #     if getattr(self, "name", None) is not None:
    #         cls_target = None
    #         name_field = self.name
    #     else:
    #         cls_target = f"{self.cls.__module__}.{self.cls.__qualname__}" if self.cls else None
    #         name_field = None
    ...
    #                 "name": name_field,
    #                 "cls": cls_target,
    #                 "backend": self._backend.value if self._backend is not None else None,
    #                 "kwargs": deepcopy(self.kwargs),
    #             },
    #             "built": self.is_built,
    #             "opt_state": self._capture_internal_state(),
    #         },
    #     )
    #     return state
    ...
    # def set_state(self, state: dict[str, Any]) -> None:
    #     """
    #     Restore optimizer wrapper from serialized state.

    #     Notes:
    #         - Does NOT immediately construct the backend optimizer
    #         - Internal state is stashed and applied on build()

    #     """
    #     from modularml.utils.environment.environment import import_from_path

    #     # Restore construction metadata
    #     init_args = state["init_args"]
    #     backend_val = init_args.get("backend")
    #     self._backend = normalize_backend(backend_val) if backend_val is not None else None

    #     self.name = init_args.get("name")
    #     cls_target = init_args.get("cls")
    #     self.cls = import_from_path(cls_target) if cls_target is not None else None
    #     self.kwargs = deepcopy(init_args.get("kwargs", {}))

    #     # Runtime fields (optimizer not built yet)
    #     self.instance = None
    #     self.parameters = None

    #     # Stash pending optimizer internal state; will be applied in build()
    #     if state.get("built", False):
    #         self._pending_state = state.get("opt_state")
    #     else:
    #         self._pending_state = None
    ...
    #     """
    #     Construct an unbound Optimizer wrapper from serialized state.

    #     The optimizer will be fully materialized once `build()` is called
    #     with model parameters / variables.
    #     """
    #     obj = cls.__new__(cls)
    #     obj.set_state(state)
    #     return obj
