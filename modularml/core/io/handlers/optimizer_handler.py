...
class OptimizerHandler(TypeHandler[Optimizer]):
    """
    TypeHandler for Optimizer objects.

    Description:
        Encodes Optimizer configuration and internal state.
    """
    ...
    def encode_config(
        ...
        *,
        ...
        if not hasattr(obj, "get_config"):
            raise NotImplementedError("Optimizer must implement a `get_config` method.")

        # JSON-safe config from Optimizer class
        config = obj.get_config()

        # Optimizer config may hold a factory callable
        # If defined via a lambda, it cannot be safely serialized
        # This is handled in ctx.make_symbol_spec
    ...
    def decode(
        ...
        *,
        ...
        Returns:
            Optimizer: The re-instantiated object.
        ...
        # Reload factory (if want encoded in config)
        ...
        optimizer = cls.from_config(config)

        # Set Optimizer state
        if not hasattr(optimizer, "set_state"):
            msg = "Optimizer must implement a `set_state` method."
            raise NotImplementedError(msg)

        optimizer.set_state(state=state)
        return optimizer
