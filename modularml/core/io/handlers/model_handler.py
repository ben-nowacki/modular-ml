from __future__ import annotations

import json
from dataclasses import asdict
from pathlib import Path
from typing import TYPE_CHECKING, Any, ClassVar

from modularml.core.io.handlers.handler import TypeHandler
from modularml.core.io.packaged_code_loaders.default_loader import default_packaged_code_loader
from modularml.core.io.serialization_policy import SerializationPolicy
from modularml.core.io.symbol_registry import symbol_registry
from modularml.core.io.symbol_spec import SymbolSpec
from modularml.core.models.base_model import BaseModel

if TYPE_CHECKING:
    from modularml.core.io.serializer import LoadContext, SaveContext


class ModelHandler(TypeHandler[BaseModel]):
    """
    TypeHandler for BaseModel objects.

    Description:
        Encodes BaseModel config and state to disk.
        Supports:
            - Registry-backed premade models (MODELS_REGISTRY)
            - Packaged custom model classes
            - Stateful weight serialization
    """

    object_version: ClassVar[str] = "1.0"

    config_rel_path = "config.json"
    state_rel_path = "state.pkl"

    # ================================================
    # Encoding
    # ================================================
    def _get_spec(self, obj: Any, ctx: SaveContext) -> SymbolSpec:
        policy = SerializationPolicy.PACKAGED
        if symbol_registry.obj_is_a_builtin_class(obj):
            policy = SerializationPolicy.BUILTIN
        elif symbol_registry.obj_in_a_builtin_registry(
            obj_or_cls=obj,
            registry_name="model_registry",
        ):
            policy = SerializationPolicy.REGISTERED

        # Create spec (internally packages code if needed)
        return ctx.make_symbol_spec(symbol=obj, policy=policy)

    def encode(
        self,
        obj: BaseModel,
        save_dir: Path,
        *,
        ctx: SaveContext,
    ) -> dict[str, str | None]:
        """
        Encodes state and config to files.

        Args:
            obj (BaseModel):
                Object instance to encode.
            save_dir (Path):
                Directory to write encodings to.
            ctx (SaveContext, optional):
                Additional serialization context.

        Returns:
            dict[str, str | None]: Mapping of "config" and "state" keys to saved files.

        """
        file_mapping = self.encode_config(obj=obj, save_dir=save_dir, ctx=ctx)
        file_mapping.update(self.encode_state(obj=obj, save_dir=save_dir, ctx=ctx))
        return file_mapping

    def encode_config(
        self,
        obj: BaseModel,
        save_dir: Path,
        *,
        ctx: SaveContext,
    ) -> dict[str, str]:
        """
        Encodes config to a json file.

        Args:
            obj (BaseModel):
                Object to encode config for.
            save_dir (Path):
                Directory to write encodings to.
            ctx (SaveContext, optional):
                Additional serialization context.

        Returns:
            dict[str, str]: Mapping of config to saved json file

        """
        if not hasattr(obj, "get_config"):
            msg = f"{obj.__class__.__qualname__} must implement a `get_config` method."
            raise NotImplementedError(msg)

        # Config from BaseModel class
        config = obj.get_config()

        # Store spec for later reload instructions
        model_cls = config["model_class"]
        sym_spec = self._get_spec(model_cls, ctx=ctx)
        config["model_class"] = asdict(sym_spec)

        # We also need to walk the init_args
        # In the case of wrappers, they could contain class objects
        init_args = config.get("init_args")
        if init_args is not None and "model_class" in init_args:
            nested_cls = init_args["model_class"]
            sym_spec = self._get_spec(nested_cls, ctx=ctx)
            config["init_args"]["model_class"] = asdict(sym_spec)

        # Save config to json file
        with (Path(save_dir) / self.config_rel_path).open("w", encoding="utf-8") as f:
            json.dump(config, f, indent=2, sort_keys=True)

        return {"config": self.config_rel_path}

    def encode_state(
        self,
        obj: BaseModel,
        save_dir: Path,
        *,
        ctx: SaveContext,
    ) -> dict[str, str]:
        """
        Encodes object state/weights to a pickle file.

        Args:
            obj (BaseModel):
                Object to encode state for.
            save_dir (Path):
                Directory to write encodings to.
            ctx (SaveContext, optional):
                Additional serialization context.

        Returns:
            dict[str, str]: Mapping of state to saved pkl file

        """
        return super().encode_state(obj=obj, save_dir=save_dir, ctx=ctx, state_rel_path=self.state_rel_path)

    # ================================================
    # Decoding
    # ================================================
    def decode(
        self,
        cls: type[BaseModel],
        load_dir: Path,
        *,
        ctx: LoadContext,
    ) -> BaseModel:
        """
        Decodes an object from a saved artifact.

        Description:
            Instantiates an object (instantiates from config and sets state).

        Args:
            cls (type[BaseModel]):
                Load config for class.
            load_dir (Path):
                Directory to decode from.
            ctx (LoadContext, optional):
                Additional de-serialization context.

        Returns:
            BaseModel: The re-instantiated object.

        """
        config: dict[str, Any] = self.decode_config(load_dir=load_dir, ctx=ctx)
        state: dict[str, Any] = self.decode_state(load_dir=load_dir, ctx=ctx)

        # Convert symbol spec back to actual class
        config["model_class"] = symbol_registry.resolve_symbol(
            spec=SymbolSpec(**config["model_class"]),
            allow_packaged_code=ctx.allow_packaged_code,
            packaged_code_loader=lambda source_ref: default_packaged_code_loader(
                artifact_path=ctx.artifact_path,
                source_ref=source_ref,
                allow_packaged=ctx.allow_packaged_code,
            ),
        )
        # Instantiate model from config
        model_obj = cls.from_config(config)

        # Restore model state (ie weights)
        if not hasattr(model_obj, "set_state"):
            msg = f"{model_obj.__class__.__qualname__} must implement a `set_state` method."
            raise NotImplementedError(msg)

        model_obj.set_state(state=state)
        return model_obj

    def decode_config(
        self,
        load_dir: Path,
        *,
        ctx: LoadContext,
    ) -> dict[str, Any] | None:
        """
        Decodes config from a json file.

        Args:
            load_dir (Path):
                Directory to decode from.
            ctx (LoadContext, optional):
                Additional de-serialization context.

        Returns:
            dict[str, Any] | None: The decoded config data.

        """
        # Restore config.json
        cfg = super().decode_config(load_dir=load_dir, ctx=ctx, config_rel_path=self.config_rel_path)

        # Walk init_args -> may need to restore nested class objects
        init_args = cfg.get("init_args")
        if init_args is not None and "model_class" in init_args:
            nested_cls = symbol_registry.resolve_symbol(
                spec=SymbolSpec(**init_args["model_class"]),
                allow_packaged_code=ctx.allow_packaged_code,
                packaged_code_loader=lambda source_ref: default_packaged_code_loader(
                    artifact_path=ctx.artifact_path,
                    source_ref=source_ref,
                    allow_packaged=ctx.allow_packaged_code,
                ),
            )
            cfg["init_args"]["model_class"] = nested_cls

        return cfg

    def decode_state(
        self,
        load_dir: Path,
        *,
        ctx: LoadContext,
    ) -> dict[str, Any]:
        """
        Decodes state from a pkl file.

        Args:
            load_dir (Path):
                Directory to decode from.
            ctx (LoadContext, optional):
                Additional de-serialization context.

        Returns:
            dict[str, Any]: The decoded state data.

        """
        return super().decode_state(load_dir=load_dir, ctx=ctx, state_rel_path=self.state_rel_path)
